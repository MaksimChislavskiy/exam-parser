...
def _deduplicate_confusable_task_numbers(
    ...
    алфавитах, с кириллической ``З`` вместо цифры ``3`` или с оформительской
    точкой в конце номера. Для классификации это один и тот же экзаменационный
    номер, поэтому сохраняется первое вхождение, как и при обычной дедупликации
    задач в парсере.
    """
...
def _task_num_equivalence_key(value: str) -> str:
    key = task_num_match_key(value).rstrip(".")
