...
## Ручная сверка нечитаемого OCR

Если Paddle обнаружил патологический повтор и не смог надёжно восстановить
условие по уточнённым crop-ам, исходный Markdown не заменяется догадкой. Вместо
этого во внешнем хранилище данных создаётся элемент очереди:

```text
<EXAM_PARSER_DATA_ROOT>/dataset/ocr_review/<content-hash>/
├── source.png
├── original_ocr.md
├── metadata.json
└── README.txt
...
После визуальной сверки рядом создаётся `correction.md` с номером и полным
условием задачи без решения. При следующем OCR-запуске программа проверит номер,
отсутствие патологических повторов и качество текста, затем применит правку до
обращения к LLM. Ключ строится из пикселей crop-а, поэтому данные не привязаны к
имени PDF или номеру страницы и подходят как проверенные пары для будущего
OCR-датасета.
...
## Результаты пакетного запуска

Пакетный экспорт учитывает каждый входной PDF ровно в одном пользовательском
каталоге:

```text
output/export/<run_name>/
├── send/          # проверенные tasks.xlsx и используемые изображения
├── review/        # сомнительные исходные PDF и README.txt с причиной
├── archive/       # полные внутренние материалы обработки
└── manifest.csv   # единый список всех PDF и их назначения
```

Документ попадает в `review`, если обработка не завершилась, пакет был остановлен
до его запуска или надёжный ZIP для `send` создать не удалось. Частичный
`tasks.xlsx` в пользовательский review-архив не добавляется: внутри находятся
только исходный PDF и пояснение, поэтому неполный результат нельзя случайно
принять за готовый.

Для пакетного запуска, завершённого старой версией программы, `review` и
`manifest.csv` можно построить из уже существующих `batch_report.csv` и приватных
архивов без OCR и запросов к LLM:
